<?php
    ...
    if ($action != PROCESSLOGIN_ACTION && !userIsAuthorized($action))
    {
        if(!loggedIn())
        {
            $url = GetControllerScript(LOGIN_ACTION) . "&RequestedPage=" . GetRequestedURI( );
            Redirect( $url );
        }
        else
        {
    ...
    else
    {
        switch ($action)
        {
            ...
            case PROCESSROLEADDEDIT_ACTION :
                ...
                break;
            default:
                include(HOME_FILE); // default action
        }
    }
    ...
    function ProcessLogin()
    {
        ...
        if(login($username,$password)){
            ...
            $url = GetControllerScript( LOGIN_ACTION ) . "&LoginFailure&RequestedPage=" . urlencode($_POST["RequestedPage"]);
            Redirect( $url );
        }
    }
